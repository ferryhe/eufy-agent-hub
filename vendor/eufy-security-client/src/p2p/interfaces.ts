...
export interface P2PClientProtocolEvents {
  "continuous recording ranges": (channel: number, data: ContinuousRecordingRanges) => void;
  "continuous playback frame": (frame: ContinuousPlaybackFrame) => void;
...
export interface ContinuousRecordingRanges {
  begin_time: number;
  end_time: number;
  videos: Array<{ start_time: number; stop_time: number }>;
}

export interface ContinuousPlaybackFrame {
  kind: "video" | "audio";
  channel: number;
  timestamp: number;
  data: Buffer;

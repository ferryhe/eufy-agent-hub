...
export class P2PClientProtocol extends TypedEmitter<P2PClientProtocolEvents> {
  ...
  private continuousPlayback?: { deviceSN: string; channel: number };
  ...
  /** HomeBase 3 continuous timeline; times are Unix seconds, not event record IDs. */
  public queryContinuousRecordings(deviceSN: string, channel: number, begin: number, end: number): void {
    if (!Number.isSafeInteger(begin) || !Number.isSafeInteger(end) || begin < 0 || end <= begin)
      throw new RangeError("Invalid continuous recording time range");
    this.sendCommandWithStringPayload({
      commandType: CommandType.CMD_DOORBELL_SET_PAYLOAD, channel,
      value: JSON.stringify({ commandType: 6000, data: { device_sn: deviceSN, begin_time: begin, end_time: end } }),
    });
  }
  ...
  public startContinuousPlayback(deviceSN: string, channel: number, accountId: string, begin: number): void {
    ...
    if (this.isCurrentlyStreaming()) throw new Error("A stream is already active on this connection");
    this.continuousPlayback = { deviceSN, channel };
    ...
    if (!this.currentMessageState[message.dataType].invalidStream) {
      switch (message.commandId) {
        case CommandType.CMD_VIDEO_FRAME:
          ...
          if (message.dataType === P2PDataType.VIDEO && this.continuousPlayback) {
            this.emit("continuous playback frame", {
              kind: "video", channel: this.continuousPlayback.channel,
              timestamp: videoMetaData.videoTimestamp, data: video_data,
              keyFrame: isKeyFrame, streamType: videoMetaData.streamType,
            });
          }
        ...
        case CommandType.CMD_AUDIO_FRAME:
          ...
          if (message.dataType === P2PDataType.VIDEO && this.continuousPlayback) {
            this.emit("continuous playback frame", {
              kind: "audio", channel: this.continuousPlayback.channel,
              timestamp: audioMetaData.audioTimestamp, data: audio_data,
            });
          }
      ...
      switch (message.commandId) {
        ...
        case CommandType.CMD_NOTIFY_PAYLOAD:
          try {
            ...
            if (json !== undefined) {
              if (json.cmd === 6000 && !Device.isLockWifi(this.rawStation.device_type, this.rawStation.station_sn)) {
                const ranges = json.payload as unknown as ContinuousRecordingRanges;
                if (ranges && Number.isFinite(ranges.begin_time) && Number.isFinite(ranges.end_time) && Array.isArray(ranges.videos))
                  this.emit("continuous recording ranges", message.channel, ranges);
              }
  ...
  private endStream(datatype: P2PDataType, sendStopCommand = false): void {
    if (this.currentMessageState[datatype].p2pStreaming) {
      if (datatype === P2PDataType.VIDEO && this.continuousPlayback) {
        ...
        this.continuousPlayback = undefined;
        if (sendStopCommand) this.sendCommandWithStringPayload({
          commandType: CommandType.CMD_DOORBELL_SET_PAYLOAD, channel: playback.channel,
          value: JSON.stringify({ commandType: 6001, data: {

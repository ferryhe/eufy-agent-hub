...
const errorCodes = ['UNAUTHENTICATED', 'UNSUPPORTED_DEVICE', 'DEVICE_UNAVAILABLE', 'DEVICE_NOT_FOUND',
  ...
  'INTERNAL_ERROR', 'PARTIAL_RECORDING', 'EXPORT_FAILED', 'JOB_CANCELLED'];
...
const contract = {
  ...
  definitions: {
    ...
    error: object({ error }),
    session: object({ authenticated: { type: 'boolean' }, phase: string, busy: { type: 'boolean' }, loginUrl: { const: '/login' }, verificationUrl: { const: '/verify' } }),
...
const validators = Object.fromEntries(['identity', 'window', 'export'].map(name =>

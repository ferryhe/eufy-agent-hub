...
## Interactive media ownership

`acquireMedia(homeBaseId)` claims the existing active HomeBase slot and returns an idempotent release function. It rejects active or previously queued work with `SERVICE_BUSY`, including recovered queued jobs. Live, playback and range owners use this slot; direct job submissions queue behind it. Release only after every protocol, stream and child cleanup promise succeeds. `whenIdle()` includes these leases. Shut down interactive owners before awaiting export shutdown. The API conservatively rejects conflicting interactive/history admissions and legacy/session mutations. This is one resident process; do not run two owners against the same output store.

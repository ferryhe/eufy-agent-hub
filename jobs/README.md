...
`get(jobId)` returns a detached snapshot or `null`. `list()` returns detached snapshots in acceptance order. `listPage({pageSize,cursor,state,serial})` returns detached snapshots newest first, ordered by durable sequence and job ID, with an opaque next cursor. The first page freezes its upper sequence; the cursor is bound to this output root and filters. State filtering reads the current state on each page rather than making a transaction snapshot. Page size defaults to 25 and is bounded at 100. Callers cannot mutate durable state through these objects. `whenIdle()` waits for work submitted to the current instance, safely recovered queued work, and cancellation cleanup, and surfaces storage faults; it is useful for tests or a graceful drain, not required for submitting a job.

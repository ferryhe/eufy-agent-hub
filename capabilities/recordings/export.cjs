...
function runFfmpeg(args) {
  return new Promise((resolve, reject) => {
    ...
    child.on('error', error => reject(error.code === 'ENOENT'
      ? new Error('未找到 FFmpeg，请设置 EUFY_FFMPEG 为可执行文件路径，或将 FFmpeg 加入 PATH。', { cause: error })
      : error));
    child.on('close', code => code === 0 ? resolve(log) : reject(new Error(`视频处理失败：${log.slice(-2000)}`)));
  });
}

async function exportRecording(prefix, destination) {
  const info = JSON.parse(fs.readFileSync(prefix + '.json', 'utf8'));
  if (!info.complete) throw new Error('下载未确认完成。');

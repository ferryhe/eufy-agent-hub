...
export function ConfirmDialog({open,title,close,children,closeLabel}:{open:boolean;title:string;close:()=>void;children:React.ReactNode;closeLabel:string}){return <DialogPrimitive.Root open={open} onOpenChange={next=>{if(!next)close()}}><DialogPrimitive.Portal><DialogPrimitive.Overlay className="drawer-overlay"/><DialogPrimitive.Content className="drawer" aria-label={title}><div><DialogPrimitive.Close asChild><button>{closeLabel}</button></DialogPrimitive.Close><DialogPrimitive.Title asChild><h2>{title}</h2></DialogPrimitive.Title>{children}</div></DialogPrimitive.Content></DialogPrimitive.Portal></DialogPrimitive.Root>}

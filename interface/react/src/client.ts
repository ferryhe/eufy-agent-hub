export type Session = { authenticated: boolean; phase: string; captcha: string | null; busy: boolean; residentEpoch: string; loginUrl: string; verificationUrl: string; logoutUrl: string }
...
export type Playback = { sessionId:string;requestId?:string;residentEpoch?:string;state:string;verificationScope?:{serial:string};positionMs:number|null;pauseExpiresAtMs:number|null;maxPauseMs:number;cleanupComplete:boolean;stopConfirmed:boolean;allowedOperations:string[];error:{code:string;message:string}|null;window?:NormalizedWindow;media?:{url:string;contentType:string;timestampSemantics:'source-received-position';sourceReceivedPositionMs:number|null;firstFrameLatencyMs:number|null;mediaEpoch:number;frameSequence:number;decodedFrames:number;bytes:number;connected:boolean;audio:false;maxFps:5;maxWidth:960;expiresAtMs:number} }
export type PlaybackRequest = {requestId:string;residentEpoch:string;operation:'create'|'seek';state:'pending'|'succeeded'|'failed';fromSessionId:string|null;sessionId:string|null;cleanupComplete:boolean;error:{code:string;message:string}|null}
...
export const api = {
  ...
  startPlayback: (requestId:string,residentEpoch:string,input:WindowInput) => post<{playback:Playback;reused:boolean}>('/api/v1/playback-sessions', {requestId,residentEpoch,media:true,speed:1,...input}),
  playback: (sessionId:string) => json<{playback:Playback}>(`/api/v1/playback-sessions/${encodeURIComponent(sessionId)}`),
  playbackRequest: (requestId:string,residentEpoch:string) => json<{request:PlaybackRequest}>(`/api/v1/playback-requests/${encodeURIComponent(requestId)}?residentEpoch=${encodeURIComponent(residentEpoch)}`),
  pausePlayback: (sessionId:string) => post<{playback:Playback}>(`/api/v1/playback-sessions/${encodeURIComponent(sessionId)}/pause`, {}),
  resumePlayback: (sessionId:string) => post<{playback:Playback}>(`/api/v1/playback-sessions/${encodeURIComponent(sessionId)}/resume`, {}),
  seekPlayback: (sessionId:string,requestId:string,residentEpoch:string,input:WindowInput) => post<{playback:Playback}>(`/api/v1/playback-sessions/${encodeURIComponent(sessionId)}/seek`, {requestId,residentEpoch,...withoutSerial(input)}),
  closePlayback: (sessionId:string) => post<{playback:Playback}>(`/api/v1/playback-sessions/${encodeURIComponent(sessionId)}/close`, {}),

...
test('Chromium assistant-ui restores origin preferences and shares one registered job and player without poll side effects',async t=>{
  ...
  const player=page.locator('[data-type="player"] video');await player.waitFor();await player.evaluate(element=>{element.dataset.identity='registered-player';element.currentTime=1.25});const playerView=page.locator('[data-type="player"]');await playerView.getByRole('button',{name:'Pin'}).click();await page.locator('[data-type="job-card"]').getByRole('button',{name:'Pin'}).click();await playerView.getByRole('button',{name:'Move down'}).click();assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=390),true);
  ...
  const before=model.calls.length;await zhDialog.getByRole('button',{name:'关闭'}).click();assert.equal(await player.getAttribute('data-identity'),'registered-player');assert.equal(await player.evaluate(element=>element.currentTime),1.25);const order=await page.locator('#workspace-views .workspace-view').evaluateAll(nodes=>nodes.map(node=>node.dataset.view));await page.reload();await page.locator('[data-type="player"] button[aria-pressed="true"]').waitFor();assert.deepEqual(await page.locator('#workspace-views .workspace-view').evaluateAll(nodes=>nodes.map(node=>node.dataset.view)),order);await page.waitForTimeout(1700);assert.equal(model.calls.length,before);assert.equal(f.calls.capture,1);assert.equal(requests.filter(item=>item.route==='/interface/agent/export').length,0);assert.ok(requests.filter(item=>item.route==='/interface/agent/state').every(item=>item.method==='GET'));

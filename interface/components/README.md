...
The [workspace registry](../workspace/workspace.mjs) binds these shared functions to four reference types. It is mounted once in the shared main area used by both modes, preserving video identity while users pin and reorder results. Timeline receipt export uses the same resident handler in both modes; fixed event timelines retain their existing event operation.

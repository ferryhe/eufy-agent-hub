...
const { DeviceType, ResponseErrorCode: Code } = require('../../adapters/eufy');
...
async function fixture(options = {}) {
  ...
  const session = new LocalEufySession(() => ({ init: async () => {}, estimateDomain: async () => {},
    hasValidSession: () => valid, login: async (_email, _password, verifyCode, captcha) => {
      if (options.challenge && !captcha && !verifyCode) return { code: Code.LOGIN_NEED_CAPTCHA };
      if (options.challenge && !verifyCode) return { code: Code.CODE_NEED_VERIFY_CODE };
      return { code: 0 };
  ...
  const server = createServer({ port: 0, session, agent: { statePath: path.join(directory, 'interface.json'), model: options.model }, recordings: options.recordings || { close() {} }, outputRoot: directory,
    ...
    exports: { outputRoot: path.join(directory, 'jobs'),
      execute: async (_executable, args, { stage, directory: jobDirectory }) => {
        ...
        if (stage === 'convert') options.media ? fs.copyFileSync(options.media, args.at(-1)) : fs.writeFileSync(args.at(-1), 'synthetic-media-bytes');
